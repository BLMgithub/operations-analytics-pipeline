...
def test_apply_contract_duplicate_on_transactional_detail(tmp_path):
    ...
    df = pd.DataFrame({
            'order_id': ['o1', 'o1', 'o2', 'o3'], # No duplicates due to pk order_id and payment_sequential
...
def test_apply_contract_cascade_drop_with_order_id(tmp_path):
    ...
    report_df_payments, new_invalid = apply_contract(run_context, 'df_payments', invalid_ids)
    report_df_order_items, new_invalid = apply_contract(run_context, 'df_order_items', invalid_ids)
